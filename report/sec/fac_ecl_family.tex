...
\input{tab/fac_ecl_final_evaluation}
...
\input{tab/fac_ecl_exdark_category}
...
\input{tab/fac_ecl_mills_illumination}
...
\input{fig/fac_ecl_qualitative/fac_ecl_qualitative}
...
The results of FAC-ECL show that frequency-aware supervision and exposure consistency improve paired reconstruction, especially on MILLs. However, the qualitative analysis also shows that several failures remain. Therefore, the next mitigation should not only improve the average output quality, but should also help the model focus on the regions where the first enhancement is more likely to be unreliable.

A possible direction would be to add a Retinex-inspired branch, explicitly separating illumination and reflectance as in Retinex-based low-light enhancement methods~\cite{DBLP:conf/bmvc/WeiWY018}. This would be a natural choice for low-light enhancement, because many errors are related to illumination correction. However, in this project this direction would partly overlap with the previous variants. The illumination-guided U-Net already tested whether an explicit brightness cue is useful, while FAC-ECL already separated smooth illumination and color information from local detail information. A full Retinex branch would also require additional assumptions, since no ground-truth illumination or reflectance maps are available.

For this reason, the next variant follows a different direction. Instead of explicitly decomposing the image, it uses the model's own photometric instability as a cue for refinement. The intuition is that, if a small exposure, gamma, color, or noise perturbation produces a large change in the enhanced output, that region may be fragile for the model. This signal is not assumed to be a perfect error map, because some errors can be stable and systematic. However, it can still provide useful information about local sensitivity.

Based on this motivation, the next family is introduced as Exposure-Consistency-based Failure-Aware Refinement (ECL-FAR). This variant is proposed in this work as a personal contribution to the pipeline, with the goal of using photometric sensitivity not only as a regularization signal, but also as practical information for local refinement. In this way, the transition from FAC-ECL to ECL-FAR remains incremental: FAC-ECL tries to reduce instability under exposure-related perturbations, while ECL-FAR uses the remaining instability as an explicit cue to refine difficult regions.